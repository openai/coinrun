...
public:
  ...
  void remove_traces_add_monsters(bool easy)
  {
    maze->monsters.clear();
  ...
  void generate_test_level()
  {
    ...
    remove_traces_add_monsters(false);
  }

  void generate_coins_on_platforms(bool easy)
  {
    ...
    remove_traces_add_monsters(easy);
    place_coins();
  }
...
void state_reset(const std::shared_ptr<State>& state, int game_type)
{
  ...
  int level_seed = 0;
  if (LEVEL_SEED_START != -1 && LEVEL_SEED_END != -1) {
    level_seed = global_rand_gen.randint(LEVEL_SEED_START, LEVEL_SEED_END);
  } else {
  ...
  if (game_type==CoinRunPlatforms_v0 || game_type==CoinRunPlatformsEasy_v0) {
    maze_gen.generate_coins_on_platforms(game_type==CoinRunPlatformsEasy_v0);
  ...
  bool recon, bool lasers)
{
  ...
  if (maze_render) {
    ...
    if (for_humans) {
      QRectF dst2 = QRectF(kx * agent->x + dx - 4*kx, WINH - ky * (agent->y+1) + dy + ky - 4*ky, 9 * kx, 9 * ky);
      p.fillRect(dst2, QColor(0, 255, 199, 100));
    }
  } else {
...
void initialize_args(int *int_args) {
  ...
  LEVEL_SEED_END = int_args[2];
  PAINT_VEL_INFO = int_args[3] == 1;
  USE_RANDOM_BLOTCHES = int_args[4] == 1;

  int rand_seed = int_args[5];
...
extern "C" void test_main_loop()
{
  ...
  int handle = vec_create(CoinRunToTheRight_v0, 1, 0, false, 4.0);
