...
class Model(object):
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        OLDVPRED = tf.placeholder(tf.float32, [None, 1])
        ...
        scaled_lr = LR * (1.0 / (1.0 - Config.get_test_frac()))

        if Config.SYNC_FROM_ROOT:
            trainer = MpiAdamOptimizer(MPI.COMM_WORLD, learning_rate=scaled_lr, epsilon=1e-5)
        else:
            trainer = tf.train.AdamOptimizer(learning_rate=scaled_lr, epsilon=1e-5)
...
class Runner(AbstractEnvRunner):
    ...
    def run(self):
        ...
        for _ in range(self.nsteps):
            ...
            mb_rewards.append(np.stack([rewards], axis=-1))
        ...
        for t in reversed(range(self.nsteps)):
            ...
            nextnonterminal = np.stack([nextnonterminal] * num_rewards, axis=-1)
...
def learn(*, policy, env, nsteps, total_timesteps, ent_coef, lr,
    ...
    if Config.LOG_ALL_MPI:
        mpi_nbatch = nbatch
    else:
        mpi_nbatch = mpi_size * nbatch
        ...
        if states is None: # nonrecurrent version
            ...
            for _ in range(noptepochs):
                ...
                for start in range(0, nbatch, nbatch_train):
                    ...
                    mblossvals.append(model.train(lrnow, cliprangenow, v_scale, *slices))
            ...
            for _ in range(noptepochs):
                ...
                for start in range(0, nenvs, envsperbatch):
                    ...
                    mblossvals.append(model.train(lrnow, cliprangenow, v_scale, *slices, mbstates))
